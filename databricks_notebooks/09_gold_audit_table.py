...
GOLD_MODEL_COMPARISON = f"{CATALOG_NAME}.{SCHEMA_NAME}.gold_model_comparison"
...
GOLD_AUDIT_TABLE = f"{CATALOG_NAME}.{SCHEMA_NAME}.gold_audit_table"
...
try:
    df_drift = spark.read.table(GOLD_DRIFT_METRICS).toPandas()
...
try:
    df_retrain = spark.read.table(GOLD_MODEL_COMPARISON).toPandas()
...
try:
    df_hallucination = spark.read.table(GOLD_HALLUCINATION_COST).toPandas()
...
# Load applicant data
df_applicants = spark.read.table(SILVER_GERMAN_CREDIT).select(*PROXY_FEATURES).toPandas().dropna()
...
spark_audit = spark.createDataFrame(df_audit)
spark_audit.write.format("delta").mode("overwrite").saveAsTable(GOLD_AUDIT_TABLE)
