...
df_home_bronze = spark.read.table(BRONZE_HOME_CREDIT)
df_german_bronze = spark.read.table(BRONZE_GERMAN_CREDIT)
...
# df_home_silver.cache() # Removed: Not supported on Serverless Web Compute
...
df_german_silver = df_german_bronze.select(
    ...
    # ── FEATURE 2: Credit Amount ──
    col("Credit_amount").cast(DoubleType()).alias("credit_amount"),
    ...
    spark_round(
        col("Credit_amount").cast(DoubleType()) / col("Duration").cast(DoubleType()),
        2
    ).alias("income_proxy"),
    ...
    spark_round(
        col("Credit_amount").cast(DoubleType()) / 
        (col("Credit_amount").cast(DoubleType()) / col("Duration").cast(DoubleType()) * lit(12)),
        4
    ).alias("financial_stress_index"),
...
# df_german_silver.cache() # Removed: Not supported on Serverless Web Compute
...
df_home_silver.write.format("delta").mode("overwrite").saveAsTable(SILVER_HOME_CREDIT)
print(f"  ✅ Saved to Unity Catalog: {SILVER_HOME_CREDIT} ({home_silver_count:,} rows)")

df_german_silver.write.format("delta").mode("overwrite").saveAsTable(SILVER_GERMAN_CREDIT)
print(f"  ✅ Saved to Unity Catalog: {SILVER_GERMAN_CREDIT} ({german_silver_count:,} rows)")
